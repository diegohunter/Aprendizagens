...
namespace GenericCrud.Util
{
    ...
    public static class StringUtil
    {
        ...
        public static decimal? TruncateDecimal(decimal? value)
        {
            if(value != null)
            {
                string strValue = string.Format("{0: 0.00}", value);
                decimal newDecimalValue = -1;
                decimal.TryParse(strValue, out newDecimalValue);
        ...
        public static string PreencherZeroEsquerda(string strValue, int lenth)
        {
        ...
        public static string RetirarCaractereEspecial(string texto,bool retirarespaco = false)
        {
            if (!string.IsNullOrWhiteSpace(texto))            {
                ...
                {
                    texto = Regex.Replace(texto.ToUpper(), item.regex, item.subst);
                }
        ...
        public static string RetirarCaractereEspecialComTrim(string texto)
        {
            if (!string.IsNullOrWhiteSpace(texto))
            {
                texto = RetirarCaractereEspecial(texto);
                texto = texto.Trim();
